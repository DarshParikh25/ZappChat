...
const Navbar = () => {
    ...
    const { setLoggedIn, setExistingUser } = useContext(AppContext);
        ...
        <div className='flex justify-between items-center cursor-pointer h-fit'>
            ...
            <div className='relative md:hidden justify-center items-center'>
                ...
                <div className={`z-50 absolute ${menuOpen ? 'block' : 'hidden'} bg-[#a9a89b] text-black text-sm text-center px-5 py-3 flex flex-col w-max top-full right-0 rounded-xl mt-3`}>
                    <p onClick={() => {navigate('/profile'); setMenuOpen(false); setExistingUser(false)}} className='cursor-pointer border-b border-black pb-2 px-1'>Edit Profile</p>
                    <p className='pt-2 cursor-pointer'>Logout</p>
                </div>
            </div>
            <div className='flex gap-7 max-md:hidden justify-center items-center text-sm'>
                <p onClick={() => {navigate('/profile'); setMenuOpen(false); setExistingUser(false)}} className='cursor-pointer text-[#c7c5b6] underline underline-offset-4 hover:scale-[1.03] transition-all duration-500'>Edit Profile</p>
                <button 
